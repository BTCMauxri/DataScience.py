...
x_indexes = np.arange(len(years))       # 11 x values including 0
width = 0.25                            # The width to be used for the bars in the bar chart
...
plt.bar(x_indexes - width, AMZN, width=width, color = '#FFA500', label = '$AMZN')   

# 2nd I do the same for $TSLA
TSLA = [1.20, 1.79, 1.93, 2.33, 9.99, 14.86, 15.38,
        14.32, 20.80, 20.41, 28.30]

plt.bar(x_indexes, TSLA, width=width, color = '#FF0000', label = '$TSLA')

# Now I do the same for $NVDA
NVDA = [.46, .39, .36, .31, .40, .50, .81, 2.61,
        4.89, 3.27, 5.41]

plt.bar(x_indexes + width, NVDA, width=width, color = '#008000', label = '$NVDA')
...
plt.xticks(ticks=x_indexes, labels=years)           # Sets years to the x values
plt.title('Decade Price Action')
plt.grid(True)
plt.tight_layout()
